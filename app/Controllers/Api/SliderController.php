...
class SliderController extends ResourceController {
   ...
   public function __construct(){
        ...
        $this->message                  = "Slider Found";
        $this->messageError             = "Slider not Found";
    ...
    public function listAll(){
        ...
        if($method == 'GET'){
            ...
            $data = $this->sliderModel->where('slider.deleted_at', NULL)->paginate($size, 'default', $page);
    ...
    public function show($id = null){
        ...
        if($method === 'GET'){
            ...
            if( in_array($id, $this->validateArray, true) ){
                ...
                $this->message = 'El id de la slider no es válido.';
            ...
            $query = $this->sliderModel->where('slider.id', $id)->where('slider.deleted_at', NULL);
    ...
    public function create(){
        ...
        if($queryResponseImage["status"]){
            ...
            $responseQuery = $this->sliderModel->insert($data);

            if($responseQuery){
                
                $newId = $this->sliderModel->insertID();
                ...
                $response = array('status'   => false,
                                  'message'  => 'Slider not was create successfull',
                                  'data'     => "",
                                  'code'      => 500);
    ...
    public function update($id = null){
        ...
        if(!$validate){
            ...
            $this->message = 'El ID Del slider no existe.';
        ...
        if($image == null){
            ...
            $responseQuery = $this->sliderModel->set($data)->where('id', $id)->update();
            ...
            if($responseQuery){
                ...
                $response = array('status'   => false,
                                    'message'  => 'Slider not was Update successfull',
                                    'data'     => "",
                                    'code'      => 500);
    ...
    public function saveImage($image = null){
        ...
        if ($image->isValid() && !$image->hasMoved()) {
            ...
            if ($validated) {
                ...
                $image->move(ROOTPATH . 'public/assets/img/slider/horizontal/', $newName);
               
                // Preparar los datos para guardar en la base de datos
                $dataImage =  '/assets/img/slider/horizontal/'.$newName;
    ...
    public function validateRegister($id = null){
        ...
        if($id){
            ...
            $queryValidate = $this->sliderModel->find($id);
    ...
    public function validateImage($id = null){
        ...
        if($id){
            ...
            $queryValidate = $this->sliderModel->where("id_subcategories", $id)->first();
    ...
    public function delete($id = null){
        ...
        if($responseQuery){
            ...
            $responseQueryDeleteImage = $this->sliderModel->where("id", $id)->delete();
            
            if($responseQueryDeleteImage){

                $response = array("status" => true, "message" => "Slider delete successfull", "data" => "", "code" => 200);
            
                return $this->respond($response);

            }else{

                $response = array("status" => true, "message" => "Error in delete the Slider", "data" => "", "code" => 404);
            ...
            $response = array("status" => true, "message" => "Slider not found", "data" => "","code" => 404);
            return $this->respond($response);
